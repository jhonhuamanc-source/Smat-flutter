...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    # Creamos el token usando la función del archivo auth.py
    access_token = auth.create_access_token(data={"sub": form_data.username})
    return {"access_token": access_token, "token_type": "bearer"}

# RUTA PROTEGIDA: Solo accesible si Flutter envía el token
@app.post("/estaciones/", status_code=201)
def crear_estacion(
    estacion: EstacionCreate, 
    user: str = Depends(auth.get_current_user) # <--- Aquí conectamos la seguridad
):
    # Si llega aquí, es porque auth.get_current_user validó el token de Flutter
    return {"msj": "Estación guardada", "usuario": user, "data": estacion}
